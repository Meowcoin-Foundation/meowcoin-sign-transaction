...
const MEOWCOIN = coininfo.mewc.main.toBitcoinJS();
const { getRPC, methods } = require("@meowcoin-foundation/meowcoin-rpc");

const full = require("./mock/full.json").debug;
const UTXOs = full.mewcUTXOs.concat(full.assetUTXOs);

const txHex = full.rawUnsignedTransaction;
const tx = bitcoin.Transaction.fromHex(txHex);

const txb = bitcoin.TransactionBuilder.fromTransaction(tx, MEOWCOIN);
...
const rpc = getRPC("anon", "anon", "https://mewc-rpc-mainnet.mewccrypto.com/rpc");
...
function getKeyPairByAddress(address) {
  ...
  const keyPair = bitcoin.ECPair.fromWIF(wif, MEOWCOIN);
  return keyPair;
}
